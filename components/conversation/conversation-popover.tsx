...
export default function ConversationPopover({friends,position,setOpen}: ConversationPopoverProps) {
    ...
    return (
      ...
      >
        <>
          ...
          <div className="h-[200px] max-w-[400px] overflow-y-auto px-2 hover-scrollbar">
            {filterFriends?.length === 0 ? (
              ...
              filterFriends?.map((friend, index) => (
                <ListItem key={index}>
                  ...
                  <p className="text-xs  text-gray-500">{friend?.profile.username}</p>
                </ListItem>
              ))
            )}
          </div>
